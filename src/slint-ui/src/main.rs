...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    window.set_always_glide(params.always_glide);
    ...
    window.on_always_glide_edited({
        let weak = window.as_weak();
        let synth = synth.clone();
        move |always| {
            let Some(window) = weak.upgrade() else { return };
            if show_result(&window, synth.set_always_glide(always)) {
                window.set_always_glide(always);
            }
        }
    });
    ...
    window.on_pitch_bend_edited({
        let weak = window.as_weak();
        let synth = synth.clone();
        move |amount| {
            let Some(window) = weak.upgrade() else { return };
            if show_result(&window, synth.set_pitch_bend(amount)) {
                window.set_pitch_bend(amount);
            }
        }
    });
    window.on_pitch_bend_range_edited({
        let weak = window.as_weak();
        let synth = synth.clone();
        move |semitones| {
            let Some(window) = weak.upgrade() else { return };
            if show_result(&window, synth.set_pitch_bend_range(semitones)) {
                window.set_pitch_bend_range(semitones);
            }
        }
    });
    ...
    window.on_noise_edited({
        ...
        move |percent| {
            let Some(window) = weak.upgrade() else { return };
            if show_result(&window, synth.set_noise(percent / 100.0)) {
                window.set_noise(percent);
            }
        }
    });
