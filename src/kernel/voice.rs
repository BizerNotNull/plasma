...
//! use the previous control tick's ENV/LFO outputs, preventing algebraic loops.
...
pub const TARGET_COUNT: usize = 36;
pub const GLOBAL_COUNT: usize = 8;
pub const GLOBAL_DEFAULTS: [f32; GLOBAL_COUNT] = [0.01, 0.2, 0.7, 0.4, 1.0, 0.0, 18000.0, 0.1];
...
pub fn target_range(target: usize) -> Result<(f32, f32, bool), Error> {
    let range = match target {
        ...
        27 | 30 | 33 | 35 => (0.0, 1.0, false),
        28 | 29 | 31 => (0.001, 10.0, true),
        32 => (0.01, 30.0, true),
...
pub struct VoiceParams {
    ...
    pub volume: f32,
    /// Attack, decay, sustain, release, LFO Hz, LFO phase, cutoff Hz, resonance.
    pub globals: [f32; GLOBAL_COUNT],
    pub lfo_wave: LfoWave,
    pub lfo_retrigger: bool,
    /// [source: ENV=0 / LFO=1][destination]. Zero removes a route.
    pub routes: [[f32; TARGET_COUNT]; 2],
}
...
impl Voice {
    ...
    /// Retriggers ENV from its current level (no discontinuity). Free LFO keeps
    /// its running phase; retrigger LFO starts at the effective phase knob.
    ...
    pub fn note_off(&mut self) {
        if self.stage != Stage::Idle && self.stage != Stage::Release {
            self.release_start = self.telemetry.env;
    ...
    pub(crate) fn next_frame_unclipped(&mut self) -> [f32; 2] {
        ...
        self.clock -= 1;
        let [attack, decay, sustain, release, rate, phase, _, _] = self.effective_globals;
        let env = &mut self.telemetry.env;
        match self.stage {
            Stage::Idle => *env = 0.0,
            Stage::Attack => {
                *env = (*env + 1.0 / (attack * self.sample_rate as f32)).min(1.0);
                if *env >= 1.0 {
                    self.stage = Stage::Decay;
                }
            }
            Stage::Decay => {
                *env = (*env - (1.0 - sustain) / (decay * self.sample_rate as f32)).max(sustain);
                if *env <= sustain {
                    self.stage = Stage::Sustain;
                }
            }
            Stage::Sustain => {
                *env += (sustain - *env) * self.smooth as f32;
            }
            Stage::Release => {
                *env = (*env - self.release_start / (release * self.sample_rate as f32)).max(0.0);
                if *env <= 0.0 {
                    self.stage = Stage::Idle;
                }
            }
